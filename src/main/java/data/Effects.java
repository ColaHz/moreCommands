...
public class Effects {
  ...
  public static void init() {
    for (java.lang.reflect.Field f : mindustry.content.Fx.class.getDeclaredFields()) {
      try {
        if (f.get(null) instanceof Effect) effects.add(new Effects((Effect) f.get(null), f.getName())); 
    ...
    effectLoop = new Timer.Task() {
      @Override
      public void run() {
        TempData.each(d -> d.hasEffect, d -> 
          Call.effectReliable(d.effect.effect, d.player.x, d.player.y, 10, arc.graphics.Color.green)
        );
      }
    };
